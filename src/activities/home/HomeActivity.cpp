...
int HomeActivity::getMenuItemCount() const {
  int count = 5;  // My Library, Recents, File transfer, Pomodoro, Settings
...
void HomeActivity::loop() {
  ...
  if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
    ...
    if (selectorIndex < recentBooks.size()) {
      ...
      onFileTransferOpen();
    } else if (menuSelectedIndex == pomodoroIdx) {
      onPomodoroOpen();
...
void HomeActivity::render() {
  ...
  std::vector<const char*> menuItems = {"Browse Files", "Recents", "File Transfer", "Pomodoro", "Settings"};
  if (hasOpdsUrl) {
