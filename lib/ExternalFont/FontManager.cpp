...
uint32_t FontManager::getSelectedFontSignature() const {
  if (_selectedIndex < 0 || _selectedIndex >= _fontCount || !_activeFont.isLoaded()) {
    return 0;
  }

  return hashString(_fonts[_selectedIndex].filename);
}
